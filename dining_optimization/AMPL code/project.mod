...
# Demand
   param d {ITEM,PERIOD};  # demand for item type i for period t (units: item)
...
minimize Cost: 
   sum{t in PERIOD} (S*z[t] + sum{i in ITEM} (s[i]*y[i,t] + h[i]*I[i,t]) );
...
# Initial demand satisfaction
   subject to DemandInitial {i in ITEM}: x[i,1] + Io[i] - I[i,1] = d[i,1];

# Demand satisfaction
   subject to Demand{i in ITEM, t in PERIOD: t>1}:
	             x[i,t] + I[i,t-1] - I[i,t] = d[i,t];
