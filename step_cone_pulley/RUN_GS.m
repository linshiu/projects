...
'LargeScale','off','MaxFunEvals',3000,'MaxIter',1000,'Diagnostics','off'); 
...
problem = createOptimProblem('fmincon','objective',@(x)FUN(x,P),'x0',x0,...
    'lb',lb,'ub',ub,'nonlcon',@(x)NONLCON(x,P),'options',options);

gs = GlobalSearch('Display','iter','PlotFcns',{@gsplotbestf,@gsplotfunccount});
